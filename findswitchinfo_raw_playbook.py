...
def switch_info_finder(connection, switch, command, grepping):
    ...
    if command=="sh ip int br":
        ansible_command = f"ansible-playbook -i /root/hosts1 shipintbr_raw.yml -u ********** -k  {grepping}"
    elif command=="sh ver" or command=="sh version":
        ansible_command = f"ansible-playbook -i /root/hosts1 shver_raw.yml -u ********** -k  {grepping}"
    elif command=="sh int status":
        ansible_command = f"ansible-playbook -i /root/hosts1 shintstatus_raw.yml -u ********** -k  {grepping}"
    elif 'mac' in command or 'address-table' in command:
        ansible_command = f"ansible-playbook -i /root/hosts1 getmac_raw.yml -u ********** -k  {grepping}"
    elif 'cdp' in command or 'nei' in command or 'neighbors' in command or 'neighbor' in command:
        ansible_command = f"ansible-playbook -i /root/hosts1 shcdpnei_raw.yml -u ********** -k  {grepping}"
